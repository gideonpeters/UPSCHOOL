...
export default {
  ...
  methods: {
    ...
    curriculumType(typeId) {
      return this.$store.state.curriculum_types.find(item => item.id == typeId);
    },
    isCompleted() {
      return true;
    },
    selectItem(v) {
      ...
      if (v.value) {
        if (this.totalUnitsSelected + v.item.credit_unit > totalRegisterable) {
          v.value = false;
          this.selectedCourses.splice(this.selectedCourses.indexOf(v.item), -1);
        ...
        if (this.totalUnitsSelected + v.item.credit_unit <= totalRegisterable) {
          if (
            !(
              this.$store.state.selectedCourses.filter(el => el.id == v.item.id)
                .length > 0
            )
          ) {
    ...
    async save(add) {
      try {
        ...
        let res = await this.$store.dispatch("addCourseToCurriculum", payload);
        if (res.status) {
