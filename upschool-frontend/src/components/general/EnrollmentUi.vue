            ...
            <v-tabs>
              ...
              <v-tab-item>
                <v-container>
                  ...
                  <v-card class="pa-4" v-if="enrollments.length == 0">
                    <div>You have not enrolled for any courses yet.</div>
                  </v-card>
                </v-container>
              </v-tab-item>
            </v-tabs>
...
export default {
    ...
    pendingHeaders() {
      ...
      if (!this.isAddingAndDropping) {
        res = [
          ...
          {
            text: " SEMESTER",
            value: "curriculum_item.curriculumable.semester_type.short_title"
          }
        ];
      } else {
        res = [
  ...
  methods: {
    ...
    dropCourse(item) {
      ...
      if (droppedItem) {
        this.pendingAddableItems = [...this.pendingAddableItems, droppedItem];
        ...
        this.pendingAddableItems = [...this.pendingAddableItems, res[0]];
      }
    },
    async saveEnrolled() {
      ...
      try {
        ...
        if (this.isAddingAndDropping) {
          ...
          if (res.status) {
            ...
            (this.snackbar.text = res.message), (this.snackbar.isActive = true);
          } else {
            (this.snackbar.text = res.message), (this.snackbar.isActive = true);
          }
    ...
    selectItem(v) {
      ...
      if (v.value) {
        if (
          this.totalUnitsSelected + v.item.curriculum_item.credit_unit >
          totalRegisterable
        ) {
          v.value = false;
        ...
        if (
          ...
          if (
            ...
            this.$store.state.selectedCourses.push(v.item.curriculum_item);
          }
        }
      } else {
        if (
          ...
          this.$store.state.selectedCourses.splice(
            this.$store.state.selectedCourses.indexOf(v.item.curriculum_item),
            1
          );
        }
      }
    ...
    async getCurrentEnrollment() {
      try {
        ...
        let data = await this.$store.dispatch("getCurrentEnrollment", id);
    ...
    async getEnrollmentHistory() {
      try {
        ...
        let data = await this.$store.dispatch("getStudentEnrollments", id);
