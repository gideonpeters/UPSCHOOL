        ...
        <v-card>
          ...
          <v-card-actions>
            ...
            <v-btn
              color="blue darken-1"
              text
              @click="dialog = false, dialogSubmit = true"
  ...
  computed: {
    items() {
      let res = [
        ...
        {
          type: "Duration:",
          value: `${this.moment(this.subsection.submission_list.due_date).from(
            this.subsection.submission_list.open_date,
            true
          )}`
        },
  ...
  methods: {
    ...
    async submit() {
      try {
        ...
        body.append("submission_list_id", this.subsection.submission_list.id);
