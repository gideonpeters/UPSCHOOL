      ...
      <v-row>
        ...
        <v-col cols="12" v-if="isAllocating">
          <v-btn
            ...
            tile
            outlined
            @click="isAllocating = false, selectedRoom = []"
          >CANCEL</v-btn>
        </v-col>
        <v-col cols="12" v-if="!isAllocating">
          <v-card class="pa-3">
            ...
            <v-data-table :headers="currentHeaders" :items="currentRoom" hide-default-footer>
              ...
              <template v-slot:item.status="{item}">
                <v-chip
                  :color="item.room && item.room.status ? 'red accent-3' : 'green accent-4'"
                  small
                  class="text-uppercase white--text"
                >{{item.room &&item.room.status ? 'Unavailable' : 'Available'}}</v-chip>
              </template>
  ...
  methods: {
    ...
    async getAvailableRooms() {
      try {
        ...
        let res = await Axios.get(`room?status=available&student_id=${id}`);
        if (res.data.status) {
